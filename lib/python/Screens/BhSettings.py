...
from Components.config import getConfigListEntry, config, ConfigYesNo, ConfigText, ConfigSelection, ConfigClock, NoSave, configfile, ConfigInteger, ConfigBoolean
...
from Screens.BhUtils import DeliteGetSkinPath, nab_Detect_Machine, BhU_get_Version
...
class DeliteSettings(Screen):
    ...
    def KeyOk(self):
	    ...
	    self.session.open(OSD3DSetupScreen)
            ...
            from Screens.BhDevice import DeliteDevicesPanel
	    self.session.open(DeliteDevicesPanel)
            ...
            self.session.open(Bh_UsbFormat)
        elif self.sel == 10:
